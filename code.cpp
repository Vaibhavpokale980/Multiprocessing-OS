...
char BUFF[40]; // Buffer
int SI=3; // System Interrupt
int TI=0; // Timer Interrupt
int PI=0; // Program Interrupt
int EM; // Error Mode
int VA[2]; // Virtual Address
int RA; // Real Address
int page_fault; // Page Fault
int abTerminate; // Terminate
...
int allocate()
{
    ...
    int allNum = rand()%29; // Random number between 0 to 29 frame of 10 words each = 30 frames 
...
void addressMap()
{
    if(0<=VA[0] && VA[0]<=9 && 0<=VA[1] && VA[1]<=9)
    {
        ...
        int PTE=(cpu->PTR[0]*1000+cpu->PTR[1]*100+cpu->PTR[2]*10+cpu->PTR[3]) + (va/10); // Page Table Entry
...
void Terminate() {
    ...
    output << "IC       :  " << cpu->IC[0] << cpu->IC[1] << "\n";
    output << "IR       :  " << cpu->IR[0] << cpu->IR[1] << cpu->IR[2] << cpu->IR[3] << "\n";
    output << "TTC      :  " << pcb->TTC << "\n";
    output << "LLC      :  " << pcb->LLC << "\n\n";
    abTerminate = 1;
}
